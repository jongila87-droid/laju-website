/* =====================================================================
   Laju — app.js  (MESIN BERSAMA untuk SEMUA halaman)
   ---------------------------------------------------------------------
...
(function () {
  ...
  // Bilah pengumuman/promo di atas header (set on:false untuk menyembunyikan).
  // Ganti 'id' bila teks diubah agar muncul lagi ke pengunjung yang sudah menutupnya.
  const ANNOUNCE = {
    on: true, id: 'promo-peluncuran-1',
    text: '🎉 Promo Peluncuran — hemat hingga 50% untuk pelanggan pertama Laju!',
    cta: 'Lihat Paket', link: 'index.html#harga',
  };
  ...
  const GOOGLE_CLIENT_ID = '575630585731-2h3so6u1danajg703kma73fo6big2vk8.apps.googleusercontent.com'; // Client ID Laju (akun syahrialr999) — origin lajucloud.netlify.app terdaftar di Google Cloud

  /* ===== JEMBATAN KE SISTEM BILLING (Fase 6) =====
     base KOSONG = mode PRATINJAU (checkout/login pakai simulasi lokal).
     Saat AKTIVASI (Fase 9), cukup isi base dengan URL portal billing, mis:
         base: 'https://my.lajucloud.id'
     → SEMUA tombol Beli/Checkout/Masuk/Daftar/Keranjang/Dashboard otomatis
       mengarah ke billing asli, TANPA mengubah satu pun file HTML.
     'slug' = nama produk di billing (diisi saat produk dibuat di FOSSBilling). */
  const BILLING = {
    base: '', // <-- ISI saat aktivasi, contoh: 'https://my.lajucloud.id'
    paths: { order: '/order', cart: '/cart', login: '/login', signup: '/signup', client: '/client' },
    slug: {
      pemula: 'mulai', bisnis: 'bisnis', premium: 'pro',
      'vps-starter': 'vps-starter', 'vps-pro': 'vps-pro', 'vps-max': 'vps-max',
      'ded-entry': 'dedicated-entry', 'ded-power': 'dedicated-power', 'ded-enterprise': 'dedicated-enterprise',
    },
  };
  const billingOn  = () => !!BILLING.base;
  const billingUrl = (p) => BILLING.base.replace(/\/+$/, '') + p;
  const billingOrderUrl = (planKey, cycle) =>
    billingUrl(BILLING.paths.order + '/' + (BILLING.slug[planKey] || planKey) + '?cycle=' + (cycle || 'monthly'));

  // Daftar paket — sumber data tunggal, dipakai checkout & dashboard
  const PLANS = {
    pemula:  { name: 'Mulai',  monthly: 15000, yearly: 12000, storage: '5 GB NVMe',  category: 'hosting' },
    bisnis:  { name: 'Bisnis', monthly: 39000, yearly: 31200, storage: '25 GB NVMe', category: 'hosting' },
    premium: { name: 'Pro',    monthly: 89000, yearly: 71200, storage: '75 GB NVMe', category: 'hosting' },
    // VPS / Cloud — paket berjenjang dengan sumber daya khusus
    'vps-starter': { name: 'VPS Starter', monthly: 120000, yearly: 96000,  storage: '40 GB NVMe',  category: 'vps', vcpu: '2 vCPU', ram: '2 GB RAM' },
    'vps-pro':     { name: 'VPS Pro',     monthly: 310000, yearly: 248000, storage: '80 GB NVMe',  category: 'vps', vcpu: '4 vCPU', ram: '8 GB RAM' },
    'vps-max':     { name: 'VPS Max',     monthly: 590000, yearly: 472000, storage: '160 GB NVMe', category: 'vps', vcpu: '6 vCPU', ram: '16 GB RAM' },
  ...
  function headerHTML() {
    const links = [
      ...
      { label: 'Solusi',  href: 'solusi-wordpress.html', match: 'solusi-wordpress.html|solusi-toko-online.html|solusi-developer.html|migrasi.html' },
      { label: 'Blog',    href: 'blog.html',    match: 'blog.html|artikel.html|artikel-kecepatan.html|artikel-ssl.html|artikel-domain.html|artikel-toko-online.html|artikel-plugin-wordpress.html' },
  ...
  function footerHTML() {
    ...
    <div class="container footer-news">
      <div class="fn-text">
        <h4>📬 Dapat tips &amp; promo terbaru</h4>
        <p>Langganan newsletter Laju — panduan website, tips kecepatan, &amp; promo spesial. Tanpa spam.</p>
      </div>
      <form class="news-form" id="newsForm" autocomplete="off" novalidate>
        <input type="email" id="newsEmail" placeholder="Alamat email Anda" aria-label="Email newsletter" />
      ...
      <div>
        ...
        <div class="foot-social">
          ...
          <a href="https://youtube.com/@lajucloud" target="_blank" rel="noopener" aria-label="YouTube" title="YouTube">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M23 12s0-3.2-.4-4.7a2.5 2.5 0 0 0-1.8-1.8C19.3 5 12 5 12 5s-7.3 0-8.8.5A2.5 2.5 0 0 0 1.4 7.3C1 8.8 1 12 1 12s0 3.2.4 4.7a2.5 2.5 0 0 0 1.8 1.8C4.7 19 12 19 12 19s7.3 0 8.8-.5a2.5 2.5 0 0 0 1.8-1.8C23 15.2 23 12 23 12zM9.8 15V9l5.2 3-5.2 3z"/></svg>
          </a>
          <a href="https://x.com/lajucloud" target="_blank" rel="noopener" aria-label="X (Twitter)" title="X">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M18.2 2h3.3l-7.2 8.3L22.8 22h-6.6l-5.2-6.8L5 22H1.7l7.7-8.8L1.2 2h6.8l4.7 6.2L18.2 2zm-1.2 18h1.8L7.1 3.9H5.2L17 20z"/></svg>
          </a>
          <a href="https://linkedin.com/company/lajucloud" target="_blank" rel="noopener" aria-label="LinkedIn" title="LinkedIn">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M20.4 3H3.6A.6.6 0 0 0 3 3.6v16.8a.6.6 0 0 0 .6.6h16.8a.6.6 0 0 0 .6-.6V3.6a.6.6 0 0 0-.6-.6zM8.3 18.3H5.5V9.7h2.8v8.6zM6.9 8.5a1.6 1.6 0 1 1 0-3.2 1.6 1.6 0 0 1 0 3.2zm11.4 9.8h-2.8v-4.2c0-1 0-2.3-1.4-2.3s-1.6 1.1-1.6 2.2v4.3H9.7V9.7h2.7v1.2h.04c.4-.7 1.3-1.4 2.6-1.4 2.8 0 3.3 1.8 3.3 4.2v4.6z"/></svg>
          </a>
        </div>
      </div>
      <div>
        ...
        <a href="solusi-wordpress.html">Solusi</a>
        <a href="migrasi.html">Migrasi Gratis</a>
      </div>
      <div>
        <h4>Perusahaan</h4>
    ...
    <div class="container footer-pay">
      <span class="fp-label">🔒 Pembayaran aman:</span>
      <span class="pay-badge">QRIS</span><span class="pay-badge">VISA</span><span class="pay-badge">Mastercard</span><span class="pay-badge">BCA</span><span class="pay-badge">Mandiri</span><span class="pay-badge">BNI</span><span class="pay-badge">GoPay</span><span class="pay-badge">OVO</span><span class="pay-badge">DANA</span>
    </div>

    <div class="container foot-bottom">
      <p>© <span id="year"></span> ${BRAND}. Seluruh hak cipta dilindungi. <span class="foot-demo">· Pratinjau produk — pembayaran online segera hadir.</span></p>
      <nav class="foot-legal" aria-label="Tautan legal">
        ...
        <a href="aup.html">Pemakaian Wajar</a>
        <a href="refund.html">Refund</a>
        <a href="peta-situs.html">Peta Situs</a>
      </nav>
    </div>`;
  }
  ...
  function initCartButtons() {
    document.addEventListener('click', (e) => {
      ...
      if (billingOn()) { location.href = billingOrderUrl(plan, cycle); return; } // aktivasi: langsung ke billing
      Cart.add(plan, cycle);
      toast(`${PLANS[plan].name} ditambahkan ke keranjang. 🛒`);
    });
  }

  // Jembatan billing: bila BILLING.base diisi (aktivasi), alihkan semua tautan
  // Beli/Checkout/Masuk/Daftar/Keranjang/Dashboard ke portal billing asli.
  function wireBillingBridge() {
    if (!billingOn()) return; // mode pratinjau: biarkan alur simulasi lokal apa adanya
    // 1) Tombol "Beli Sekarang" (checkout.html?plan=KEY&cycle=...)
    $$('a[href^="checkout.html"]').forEach((a) => {
      const q = new URLSearchParams((a.getAttribute('href').split('?')[1]) || '');
      const plan = q.get('plan'); if (!plan) return;
      a.setAttribute('href', billingOrderUrl(plan, q.get('cycle') || 'monthly'));
    });
    // 2) Tautan akun & keranjang
    const map = { 'masuk.html': BILLING.paths.login, 'daftar.html': BILLING.paths.signup, 'keranjang.html': BILLING.paths.cart };
    $$('a[href]').forEach((a) => {
      const href = a.getAttribute('href');
      if (map[href]) a.setAttribute('href', billingUrl(map[href]));
      else if (href === 'dashboard.html' || href.startsWith('dashboard.html#')) a.setAttribute('href', billingUrl(BILLING.paths.client));
    });
  }
  ...
  function getAIReply(text) {
    ...
    if (has('harga', 'biaya', 'tarif', 'paket', 'berapa', 'murah', 'price'))
      return 'Pilihan paket kami:<br>• <strong>Mulai</strong> — Rp15.000/bln<br>• <strong>Bisnis</strong> — Rp39.000/bln (terpopuler ⭐)<br>• <strong>Pro</strong> — Rp89.000/bln<br>Hemat 20% bila bayar tahunan. Lihat <a href="index.html#harga">halaman Harga</a> 👉';
    if (has('domain', '.com', '.id', '.net', '.store'))
      return 'Untuk domain, ketik nama impianmu di kotak pencarian <a href="index.html#beranda">beranda</a> untuk cek ketersediaan. Harga: .id Rp199.000/thn, .com Rp159.000/thn, .store Rp39.000/thn, .my.id Rp25.000/thn. 🌐';
    ...
    if (has('migrasi', 'pindah', 'migrate'))
      return 'Migrasi website dari penyedia lama <strong>GRATIS</strong> & dibantu tim kami, tanpa downtime. Lihat <a href="migrasi.html">cara migrasi</a> 🚚';
    if (has('vps', 'virtual private', 'root', 'server pribadi'))
      return 'Butuh kendali penuh? <strong>VPS</strong> kami mulai Rp120.000/bln — akses root, NVMe, & opsi Managed. Lihat <a href="vps.html">paket VPS</a> 🖥️';
    if (has('wordpress', ' wp '))
      return 'Kami pas untuk <strong>WordPress</strong>: instal 1-klik, cepat, & aman. Lihat <a href="solusi-wordpress.html">Solusi WordPress</a> 🟦';
    if (has('toko', 'jualan', 'olshop', 'online shop', 'ecommerce', 'e-commerce', 'woocommerce'))
      return 'Mau buka <strong>toko online</strong>? Hosting kami tahan ramai & checkout aman. Lihat <a href="solusi-toko-online.html">Solusi Toko Online</a> 🛒';
    if (has('developer', 'staging', ' api ', 'koding', 'ngoding'))
      return 'Untuk <strong>developer</strong>: VPS akses root, server staging (paket Pro), NVMe & PHP terbaru. Lihat <a href="solusi-developer.html">Solusi Developer</a> 💻';
    if (has('afiliasi', 'reseller', 'komisi', 'affiliate'))
      return 'Ada <strong>Program Afiliasi</strong> — dapat komisi dengan mengajak orang lain pakai Laju. Lihat <a href="afiliasi.html">halaman Afiliasi</a> 🤝';
    if (has('kontak', 'hubungi', ' cs ', 'customer', 'whatsapp', ' wa ', 'email', 'telepon', 'manusia', 'admin', 'bantuan'))
      return 'Hubungi tim kami via WhatsApp (tombol hijau 🟢), email <a href="mailto:halo@lajucloud.id">halo@lajucloud.id</a>, atau <a href="bantuan.html">Pusat Bantuan</a>. Respon < 5 menit! ⚡';
  ...
  /* =====================================================================
     7c. NEWSLETTER & BILAH PENGUMUMAN
     ===================================================================== */
  const Newsletter = {
    list: () => store.get('nh_newsletter', []),
    add(email) {
      email = (email || '').trim().toLowerCase();
      const list = Newsletter.list();
      if (!list.includes(email)) { list.push(email); store.set('nh_newsletter', list); }
      return email;
    },
  };
  function wireNewsletter() {
    ...
    f.addEventListener('submit', (e) => {
      e.preventDefault();
      const inp = $('#newsEmail'); const email = (inp.value || '').trim().toLowerCase();
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { toast('Masukkan alamat email yang valid.', 'error'); return; }
      Newsletter.add(email); inp.value = '';
      toast('Terima kasih! Anda berlangganan newsletter Laju. 📬');
    });
  }
  function initAnnounce() {
    if (!ANNOUNCE.on || store.get('nh_announce_' + ANNOUNCE.id, false)) return;
    const bar = document.createElement('div');
    bar.className = 'announce';
    bar.innerHTML = `<div class="container announce-in">
        <span class="announce-txt">${ANNOUNCE.text} <a href="${ANNOUNCE.link}">${ANNOUNCE.cta} →</a></span>
        <button class="announce-x" aria-label="Tutup pengumuman">✕</button>
      </div>`;
    document.body.insertBefore(bar, document.body.firstChild);
    bar.querySelector('.announce-x').addEventListener('click', () => {
      store.set('nh_announce_' + ANNOUNCE.id, true); bar.remove();
    });
  }
  ...
  /* =====================================================================
     ...
     ===================================================================== */
  const SEARCH_INDEX = [
    { t: 'Beranda', u: 'index.html', k: 'home utama awal', i: '🏠' },
    { t: 'Web Hosting & Harga', u: 'index.html#harga', k: 'hosting shared paket harga mulai bisnis pro', i: '💰' },
    { t: 'VPS & Cloud', u: 'vps.html', k: 'vps cloud server virtual root', i: '🖥️' },
    { t: 'Dedicated Server', u: 'dedicated.html', k: 'dedicated server fisik', i: '🗄️' },
    { t: 'Domain', u: 'index.html#domain', k: 'domain nama .id .com beli', i: '🌐' },
    { t: 'Solusi: Hosting WordPress', u: 'solusi-wordpress.html', k: 'wordpress wp blog cms', i: '🟦' },
    { t: 'Solusi: Toko Online', u: 'solusi-toko-online.html', k: 'toko online ecommerce woocommerce jualan', i: '🛒' },
    { t: 'Solusi: Developer', u: 'solusi-developer.html', k: 'developer vps staging api root', i: '💻' },
    { t: 'Migrasi Gratis', u: 'migrasi.html', k: 'migrasi pindah hosting gratis', i: '🚚' },
    { t: 'Blog', u: 'blog.html', k: 'blog artikel tips panduan', i: '📝' },
    { t: 'Artikel: Cara Memilih Hosting', u: 'artikel.html', k: 'memilih hosting pemula panduan', i: '🚀' },
    { t: 'Artikel: Mempercepat Website', u: 'artikel-kecepatan.html', k: 'cepat loading performa kecepatan optimasi', i: '⚡' },
    { t: 'Artikel: Pentingnya SSL', u: 'artikel-ssl.html', k: 'ssl https keamanan gembok', i: '🔒' },
    { t: 'Tentang Kami', u: 'tentang.html', k: 'tentang perusahaan profil', i: '🏢' },
    { t: 'Pusat Bantuan', u: 'bantuan.html', k: 'bantuan help faq tiket dukungan panduan', i: '🤝' },
    { t: 'Kontak', u: 'kontak.html', k: 'kontak hubungi alamat email whatsapp', i: '✉️' },
    { t: 'Program Afiliasi', u: 'afiliasi.html', k: 'afiliasi reseller komisi', i: '🤝' },
    { t: 'Status Sistem', u: 'status.html', k: 'status uptime gangguan sistem', i: '📊' },
    { t: 'Tes Kecepatan', u: 'tes-kecepatan.html', k: 'tes kecepatan speed test', i: '⚡' },
    { t: 'Karir', u: 'karir.html', k: 'karir lowongan kerja bergabung tim', i: '💼' },
    { t: 'Dashboard', u: 'dashboard.html', k: 'dashboard akun layanan tagihan', i: '📋' },
    { t: 'Masuk', u: 'masuk.html', k: 'masuk login', i: '🔑' },
    { t: 'Daftar', u: 'daftar.html', k: 'daftar register akun baru', i: '✍️' },
    { t: 'SLA (Jaminan Uptime)', u: 'sla.html', k: 'sla jaminan uptime', i: '📄' },
    { t: 'Kebijakan Refund', u: 'refund.html', k: 'refund garansi uang kembali', i: '📄' },
    { t: 'Pemakaian Wajar (AUP)', u: 'aup.html', k: 'aup pemakaian wajar fair use', i: '📄' },
    { t: 'Syarat & Ketentuan', u: 'syarat.html', k: 'syarat ketentuan tos', i: '📄' },
    { t: 'Kebijakan Privasi', u: 'privasi.html', k: 'privasi data pdp', i: '📄' },
    { t: 'Peta Situs', u: 'peta-situs.html', k: 'peta situs sitemap semua halaman', i: '🗺️' },
  ];
  function initSearch() {
    const btn = $('#searchBtn'); if (!btn) return;
    if ($('#searchOverlay')) return; // sudah dibuat
    const ov = document.createElement('div');
    ov.className = 'search-overlay'; ov.id = 'searchOverlay';
    ov.innerHTML = `
      <div class="search-box" role="dialog" aria-label="Pencarian situs">
        <div class="search-in">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          <input id="searchInput" type="text" placeholder="Cari halaman, panduan, atau fitur…" autocomplete="off" />
          <button id="searchClose" aria-label="Tutup">✕</button>
        </div>
    ...
    const render = (q) => {
      ...
      if (q) items = SEARCH_INDEX.filter(it => (it.t + ' ' + it.k).toLowerCase().includes(q));
      if (!items.length) { results.innerHTML = `<p class="search-empty">Tidak ada hasil untuk "<strong>${q.replace(/</g,'&lt;')}</strong>". Coba kata lain.</p>`; return; }
      results.innerHTML = (q ? '' : '<p class="search-hint">Halaman populer</p>') +
        items.slice(0, 12).map(it => `<a href="${it.u}" class="search-res"><span class="sr-ic">${it.i}</span><span>${it.t}</span></a>`).join('');
    };
    const open = () => { ov.classList.add('open'); render(''); setTimeout(() => input.focus(), 60); };
    const close = () => { ov.classList.remove('open'); input.value = ''; };
  ...
  /* =====================================================================
     7e. TOMBOL BAGIKAN (otomatis di halaman artikel)
     ===================================================================== */
  function initShare() {
    if (!/^artikel/.test(PAGE)) return;
    const head = $('.page-head .container'); if (!head) return;
    ...
    bar.innerHTML = `
      ...
      <a class="share-ic sh-wa" href="${wa}" target="_blank" rel="noopener" aria-label="Bagikan ke WhatsApp" title="WhatsApp">
        <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor"><path d="M.057 24l1.687-6.163a11.867 11.867 0 0 1-1.587-5.946C.16 5.335 5.495 0 12.05 0a11.82 11.82 0 0 1 8.413 3.488 11.82 11.82 0 0 1 3.48 8.414c-.003 6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 0 1-5.688-1.448L.057 24zM6.6 20.1c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884a9.82 9.82 0 0 0 1.51 5.26l-.999 3.648 3.978-1.607zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/></svg>
      </a>
      <a class="share-ic sh-fb" href="${fb}" target="_blank" rel="noopener" aria-label="Bagikan ke Facebook" title="Facebook">
        <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor"><path d="M22 12a10 10 0 1 0-11.5 9.9v-7H8v-2.9h2.5V9.8c0-2.5 1.5-3.9 3.8-3.9 1.1 0 2.2.2 2.2.2v2.5h-1.2c-1.2 0-1.6.8-1.6 1.6V12H16l-.4 2.9h-2.1v7A10 10 0 0 0 22 12z"/></svg>
      </a>
      <a class="share-ic sh-tw" href="${tw}" target="_blank" rel="noopener" aria-label="Bagikan ke X" title="X (Twitter)">
        <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><path d="M18.2 2h3.3l-7.2 8.3L22.8 22h-6.6l-5.2-6.8L5 22H1.7l7.7-8.8L1.2 2h6.8l4.7 6.2L18.2 2zm-1.2 18h1.8L7.1 3.9H5.2L17 20z"/></svg>
      </a>
      <a class="share-ic sh-tg" href="${tg}" target="_blank" rel="noopener" aria-label="Bagikan ke Telegram" title="Telegram">
        <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor"><path d="M21.9 4.3l-3 14.1c-.2 1-.8 1.2-1.7.8l-4.6-3.4-2.2 2.1c-.2.2-.5.5-.9.5l.3-4.6 8.4-7.6c.4-.3-.1-.5-.6-.2L7.5 13.1 3 11.7c-1-.3-1-1 .2-1.4l17.2-6.6c.8-.3 1.5.2 1.5 1.6z"/></svg>
      </a>
      <button class="share-ic sh-copy" id="shareCopy" aria-label="Salin tautan" title="Salin tautan">
    ...
    $('#shareCopy').addEventListener('click', () => {
      const done = () => toast('Tautan artikel disalin! 🔗');
  ...
  /* =====================================================================
     7f. POP-UP PROMO (exit-intent, sekali per minggu, desktop)
     ===================================================================== */
  function initExitPromo() {
    if (window.innerWidth < 820) return; // desktop saja
    const KEY = 'nh_exitpromo_until';
    const until = store.get(KEY, 0);
    if (until && Date.now() < until) return; // sudah ditampilkan minggu ini
    let fired = false;
    const handler = (e) => {
      if (fired) return;
      if (e.clientY > 0 || e.relatedTarget) return; // hanya saat kursor keluar lewat atas
      fired = true;
      document.removeEventListener('mouseout', handler);
      store.set(KEY, Date.now() + 7 * 24 * 3600 * 1000); // jangan ganggu 7 hari
      modal({
        title: 'Tunggu dulu! 🎁',
        body: '<p style="margin:0 0 6px">Jangan lewatkan <strong>Promo Peluncuran</strong> — hemat hingga <strong>50%</strong> untuk pelanggan pertama Laju.</p><p style="margin:0;color:var(--muted);font-size:.9rem">Plus garansi uang kembali 30 hari. Tanpa risiko.</p>',
        actions: [
          { label: 'Nanti saja', variant: 'btn-outline', onClick: (close) => close() },
          { label: '🚀 Lihat Promo', variant: 'btn-primary', onClick: (close) => { close(); location.href = 'index.html#harga'; } },
        ],
      });
    };
    setTimeout(() => document.addEventListener('mouseout', handler), 4000); // aktif setelah 4 detik
  }
  ...
  window.NH = { Auth, Services, Billing, Cart, Tickets, Newsletter, PLANS, COUPONS, PPN_RATE, toast, modal, rp, store, $, $$, PAGE, GOOGLE_CLIENT_ID,
    billing: { on: billingOn, url: billingUrl, orderUrl: billingOrderUrl, config: BILLING, rewire: wireBillingBridge } };
})();
